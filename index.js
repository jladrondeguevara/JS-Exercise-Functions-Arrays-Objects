...
function makePersonObject(idIn, nameIn, emailIn) {
  let personArray = {id: idIn, name: nameIn, email: emailIn};
  return personArray;
}
...
function isItAnApple(fruitArray) {
  ...
  {
    if(fruitArray[i] == "apple"){
      isItAppleArray.push(true);
    }
    else {
      isItAppleArray.push(false);
    } 
  }
  return isItAppleArray;
}
...
function getLastCarInfo(inventoryArray) {
  return "This is a " + inventoryArray[inventoryArray.length-1].car_make + inventoryArray[inventoryArray.length-1].car_make;
}
